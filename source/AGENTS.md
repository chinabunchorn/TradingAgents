...
3. **Obsidian report adapter (write side) → PR #2 (2026-08-17)**. Env re-pointing (C1, zero code): `.env` sets `TRADINGAGENTS_RESULTS_DIR` → `<vault>/reports/`, `TRADINGAGENTS_MEMORY_LOG_PATH` → `<vault>/trading_memory.md`. Adapter (C2/C3): `tradingagents/reporting_frontmatter.py` — `parse_report_meta` (decision = PM `**Rating**`, confidence = `**Confidence:**`, tolerates colon-in/outside-emphasis), `decorate_complete_report` (YAML frontmatter `ticker`/`date`/`decision`/`confidence` + one-line `[[Trading Dashboard]]` wikilink, idempotent), `sync_hub_note` (`Trading Dashboard.md` `## Recent runs`). Wired into `write_report_tree` (single chokepoint → CLI **and** `save_reports`). Hub note lives at the reports root (`<reports>/Trading Dashboard.md`), sibling of the per-ticker run dirs. +6 tests; 591 passed, ruff clean. **CLI save path (PR #3)**: the interactive `Save report?` default now points into the vault too (`config["results_dir"]/TICKER_stamp`) instead of `cwd/reports`, so a plain-Enter save lands the report tree where the adapter + hub note live.
